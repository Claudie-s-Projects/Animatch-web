...
async function submit() {
  ...
  loading.value = true
  try {
    await $fetch('/auth/register-refuge', {
      baseURL: apiBase,
      method: 'POST',
      body: form,
    })
...
<template>
  <main class="max-w-md mx-auto px-4 py-16">
    <h1 class="text-2xl font-bold mb-8">Créer un espace refuge</h1>

    <form @submit.prevent="submit" class="space-y-4">
      <div>
        <label class="text-sm text-gray-600">Nom du refuge</label>
      ...
      <div>
        ...
        <input v-model="form.adresse" required type="text" class="w-full border rounded-lg px-3 py-2 mt-1 text-sm" />
      </div>

      <div>
        <label class="text-sm text-gray-600">Localité</label>
        <input v-model="form.localite" required type="text" class="w-full border rounded-lg px-3 py-2 mt-1 text-sm" />
      </div>

      <p v-if="error" class="text-sm text-red-500">{{ error }}</p>

      <button type="submit" :disabled="loading" class="w-full bg-blush-400 text-white py-2 rounded-lg text-sm font-medium hover:bg-blush-500 disabled:opacity-50">
